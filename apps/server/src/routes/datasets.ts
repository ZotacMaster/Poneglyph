...
import { and, count, desc, eq, asc, ilike, or, arrayContains, inArray } from "drizzle-orm";
import { Hono } from "hono";

const app = new Hono();

export const datasetsRoute = app.get("/", zValidator("query", DatasetQuerySchema), async (c) => {
  ...
  if (query.q) {
    conditions.push(
      or(ilike(datasets.title, `%${query.q}%`), ilike(datasets.description, `%${query.q}%`)),
    );
  }
